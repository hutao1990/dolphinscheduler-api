...
public class ResourcesService extends BaseService {
    ...
    public Result createResource(User loginUser,
        ...
        for (MultipartFile file : files) {
            if (file.getOriginalFilename().endsWith(".zipx")) {
                try {
    ...
    private void uploadZipFile(ZipFile zipFile, User loginUser, ResourceType type, int pid, String currentDir, Result result) throws IOException {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (entry.isDirectory()) {
                name = org.apache.commons.lang3.StringUtils.removeEnd(name, "/");
                int i = name.lastIndexOf("/");
                if (i != -1) {
                    parentPath = name.substring(0, i);
                }
                String dirName = name.substring(i + 1);
                dirs.add(new Pair<>(parentPath, dirName));
                continue;
            }
            if (name.contains("/")) {
